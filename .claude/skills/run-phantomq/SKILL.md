---
name: run-phantomq
description: Launch the PhantomQ Flask app and drive it in a real browser (home, sign up, open a room, use the Caesar widget, submit an answer) with screenshots. Use when asked to run, start, or screenshot PhantomQ, or to confirm the UI works end-to-end.
---
...
for `/healthz`, then in a headless browser: loads the home page, signs up as
`demo_player`, opens the **The Shift** room, checks the Caesar-wheel widget
rendered its live output, submits `hello world` to the first question, and
confirms the "Correct! +15 XP" result and the nav XP chip updating. It writes 5
screenshots (`1-home.png` … `5-answered.png`) to a temp dir and prints their
...
full stack works. A blank frame means the launch failed.
...
Set `PHANTOMQ_SECRET_KEY` to keep logins across restarts; otherwise a random key
is used each run (a startup line warns about this). `PHANTOMQ_PORT` and
`PHANTOMQ_DB` override the port and database file.
...
# Full flow needs a cookie jar (session): sign up, then POST an answer as JSON.
```
