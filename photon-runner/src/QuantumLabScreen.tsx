...
import { createQuantumLabWorld } from './sceneQuantumLab';
import { createKeyboardMovementInput } from './keyboardMovement';
import { Joystick } from './Joystick';
import { PerfOverlay } from './PerfOverlay';

/** The walkable Quantum Lab interior — five teaching stations plus a bridge
 * to the deeper QuantumPhenomenaLab sandbox. Station explanations render as
 * an in-world hologram panel (see sceneQuantumLab.ts), not an HTML overlay
 * — this screen's own UI stays to a station-name chip and an interact
 * prompt, same minimal footprint as CampusScreen. */
export function QuantumLabScreen({ onOpenSimulator }: { onOpenSimulator: () => void }) {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const gameRef = useRef<ReturnType<typeof createQuantumLabWorld> | null>(null);
  ...
  useEffect(() => {
    ...
    const game = createQuantumLabWorld({ onOpenSimulator, onStationChange: setStation, onPromptChange: setPrompt });
    ...
    <div className="relative w-full h-full overflow-hidden">
      ...
      <div className="absolute top-3 left-3 pointer-events-none">
        <div className="glass rounded-2xl px-3.5 py-2.5">
          <div className="label-mono">Quantum Lab</div>
          <div className="text-xs ink-2 mt-0.5">
            {station ? station : 'Walk up to a station and press E.'}
          </div>
        </div>
      </div>
