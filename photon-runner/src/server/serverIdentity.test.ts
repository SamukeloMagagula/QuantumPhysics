...
describe('guest identity', () => {
  ...
  it('sets Secure, HttpOnly and SameSite on production cookies', async () => {
    const db = openDb(':memory:');
    try {
      const productionApp = createApp({ db, secret: SECRET, secureCookies: true });
      const res = await request(productionApp).get('/api/whoami');
  ...
  it('does not expose retired heist endpoints by default', async () => {
    const res = await request(app).post('/api/heist/room').send({ mapId: 'relay' });
    expect(res.status).toBe(404);
    expect(res.body.error).toBe('Unknown API endpoint.');
  });
