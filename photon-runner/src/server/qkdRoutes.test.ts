...
describe('qkd multiplayer API', () => {
  ...
  it('rejects outsiders starting a game without changing its lobby', async () => {
    const owner = await guestCookie(app);
    const created = await request(app).post('/api/qkd/game').set('Cookie', owner).send({ role: 'alice' });
    const code = created.body.code;
    for (const cookie of [undefined, await guestCookie(app)]) {
      const req = request(app).post(`/api/qkd/game/${code}/start`);
      if (cookie) req.set('Cookie', cookie);
      expect((await req).status).toBe(403);
    }
    const state = await request(app).get(`/api/qkd/game/${code}`).set('Cookie', owner);
    expect(state.body.phase).toBe('lobby');
    expect(state.body.round).toBe(0);
  });

  it('allows a seated player to start and returns their own view', async () => {
    const alice = await guestCookie(app);
    const created = await request(app).post('/api/qkd/game').set('Cookie', alice).send({ role: 'alice' });
    const code = created.body.code;
    const bob = await guestCookie(app);
    await request(app).post(`/api/qkd/game/${code}/join`).set('Cookie', bob).send({ role: 'bob' });
    const started = await request(app).post(`/api/qkd/game/${code}/start`).set('Cookie', bob);
    expect(started.status).toBe(200);
    expect(started.body.yourRole).toBe('bob');
    expect(started.body.phase).toBe('alice_setup');
    const repeated = await request(app).post(`/api/qkd/game/${code}/start`).set('Cookie', alice);
    expect(repeated.body.round).toBe(1);
    expect(repeated.body.yourRole).toBe('alice');
  });
